...
def compare_tree_files():
    ...
    parser.add_argument(
        "-r", "--reference", dest="ref_file", help="Reference file (csv format)"
    )
    parser.add_argument(
        "-rc",
        "--reference_config",
    ...
    parser.add_argument(
        "-f", "--file", dest="tree_file", help="Tree file (csv format)", required=True
    )
    parser.add_argument(
        "-fc",
        "--file_config",
    ...
    if "1" in ns.parts:
        ...
        if len(ref_confs) != 4:
            ...
            parser.print_help()
            sys.exit(1)
        skip_lines, ref_id_col, ref_tit_col = (
            int(ref_confs[0]),
            int(ref_confs[2]),
            int(ref_confs[3]),
        )
        ...
        for i, line in enumerate(lines, start=skip_lines + 1):
            ...
            if k in ref_dic:
                ...
                if not re.match(decimal_identifier, k):
                    error(
                        "{}, bad ref identifier value '{}', '{}'".format(
                ...
                for k in code.split(","):
                    ...
                    if k not in tree_dic:
                        ...
                        if not k.startswith("A.") and not re.match(
                            ...
                            error(
                                "{},{}, bad tree identifier value '{}', '{}'".format(
                                    i, id_col, k, v
                                )
                            )
                    ...
                    if parent_code in code_ids:
                        ...
                        for level in parents:
                            if level not in code_ids:
                                ...
                                verbose(
                                    "{}, added decimal level '{}'".format(ln_nb, level)
                                )
                            else:
